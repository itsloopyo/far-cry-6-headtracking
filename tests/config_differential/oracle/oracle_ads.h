...
// What v0.1.0's reader called for its ADS keys: cameraunlock-core 2eded2c's
// cpp/include/cameraunlock/ads/ads_mode.h under this namespace, then the lines of v0.1.0's
// src/ads.h that config.h and config.cpp use. Core no longer has the header.

#include <cctype>
#include <cstring>
#include <initializer_list>

namespace FarCry6HeadTracking::oracle::ads {
...
namespace FarCry6HeadTracking::oracle {

using ads::AdsMode;
using ads::AdsModeValue;
using ads::kDefaultAdsMode;

constexpr int kDefaultVkAdsMode = 0x2D;  // VK_INSERT

inline AdsMode ParseFarCry6AdsMode(const char* text) {
    return ads::ParseAdsMode(text, false);
}
